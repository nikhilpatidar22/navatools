      ...
      <Card>
        <CardContent className="p-6 md:p-8 space-y-5">
          ...
          <Button onClick={handleConvert} disabled={!file || loading} size="lg" className="w-full">
            {loading ? <><Loader2 className="h-5 w-5 mr-2 animate-spin" /> Converting...</> : <><Download className="h-5 w-5 mr-2" /> Convert to Excel</>}
          </Button>
          {result && <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-300 text-sm font-medium">Converted {result.pages} pages successfully!</div>}
        </CardContent>
      </Card>
    </div>
  )
